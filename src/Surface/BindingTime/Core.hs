...
instance HasVar BIType where
  ...
  subst be0 x (BIType meta bityMain) =
    BIType meta $
      case bityMain of
        BITyBase bes -> BITyBase (map go bes)
        BITyArrow (Nothing, btye1) btye2 -> BITyArrow (Nothing, go btye1) (go btye2)
        BITyArrow (Just y, btye1) btye2 -> BITyArrow (Just y, go btye1) (if y == x then btye2 else go btye2)
    where
